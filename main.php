 ...
 */
	...
	if ( isset($_GET['id']) || isset($_GET['title']) || isset($_GET['category']) || isset($_GET['source']) )
	{
		...
		$request_category = $_GET['category'];
		if ($request_category != "")
		{
			$query = $query . "`category` LIKE '" . $request_category . "' AND ";
		}
		
		$request_source = $_GET['source'];
		if ($request_source != "")
		{
 ...
 */
	$id 		= $row[0]; // Entry's ID
	$date		= $row[1]; // Entry's Post Date
	$author		= $row[2]; // Author of the entry
	$category 	= $row[3]; // Category, i.e: RPG Session, IM
	$source 	= $row[4]; // Source, i.e: Classic Rollage, GG
	$title 		= $row[5]; // Custom Label
	$text		= $row[6]; // Quote
	$explicit	= $row[7]; // Whether it contains explicit vocabulary
	$text_enGB	= $row[8]; // Personally translated quote
	$rating		= $row[9]; // Fixed Rating
	...
	**/
	...
	/* Footer */
		...
		$votes_result = mysql_query($tmp_query);
		$vote = 0;	// Default current post's vote rating to 0;
		
		while ( $votes = mysql_fetch_array($votes_result, MYSQL_NUM) ) {
			$value = $votes[0];
			$vote = $vote + $value;
		}
		
		// Calculate Average
		if (mysql_num_rows($votes_result) > 0 ) {
			$rating = $vote / mysql_num_rows($votes_result);
		}
